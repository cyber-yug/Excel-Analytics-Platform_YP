...
const Analytics = () => {
  ...
  const fetchSpecificFile = async (id) => {
    try {
      setError(null);
      const response = await axios.get(`http://localhost:3001/api/upload/files/${id}`);
  ...
  const fetchStats = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3001/api/analytics/stats/${id}`);
  ...
  const generateAutoChart = async (suggestion) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3001/api/analytics/chart/${selectedFile._id}`, suggestion.config);
  ...
  const generateChartWithConfig = async (updatedConfig) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3001/api/analytics/chart/${selectedFile._id}`, updatedConfig);
  ...
  const generateChart = async () => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3001/api/analytics/chart/${selectedFile._id}`, chartConfig);
      console.log('Chart response:', response.data);
      setChartData(response.data);
    } catch (error) {
      console.error('Failed to generate chart:', error);
